...
resource "aws_api_gateway_deployment" "main" {
  ...
  triggers = {
    redeployment = sha1(join(",", list(
      ...
      jsonencode(aws_api_gateway_integration.put_metrics),
      jsonencode(aws_api_gateway_integration.events),
      jsonencode(aws_api_gateway_integration.athena_query),
      jsonencode(aws_api_gateway_integration.post_athena_query),
    )))
  }
}
...
resource "aws_api_gateway_resource" "athena_query" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  parent_id   = aws_api_gateway_resource.root.id
  path_part   = "athena"
}

resource "aws_api_gateway_method" "athena_query" {
  rest_api_id      = aws_api_gateway_rest_api.main.id
  resource_id      = aws_api_gateway_resource.athena_query.id
  http_method      = "GET"
  authorization    = "NONE"
  api_key_required = true
}
resource "aws_api_gateway_integration" "athena_query" {
  rest_api_id             = aws_api_gateway_rest_api.main.id
  resource_id             = aws_api_gateway_resource.athena_query.id
  http_method             = aws_api_gateway_method.athena_query.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.athena_query.invoke_arn
}
resource "aws_api_gateway_method" "post_athena_query" {
  rest_api_id      = aws_api_gateway_rest_api.main.id
  resource_id      = aws_api_gateway_resource.athena_query.id
  http_method      = "POST"
  authorization    = "NONE"
...
resource "aws_api_gateway_integration" "post_athena_query" {
  rest_api_id             = aws_api_gateway_rest_api.main.id
  resource_id             = aws_api_gateway_resource.athena_query.id
  http_method             = aws_api_gateway_method.post_athena_query.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.athena_query.invoke_arn
}
