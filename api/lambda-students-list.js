 ...
 */
const MULTI_DIM_METRICS = { 'Tags': { dimension: 'Tag', cache: 'tags', metric: 'Tags' }, 'Replay talks started': { dimension: 'Talk', cache: 'talks', metric: 'Replay talks started' } };
...
exports.handler = async (req, context) => {
    ...
    let metricNames = (req.multiValueQueryStringParameters.metric || [])
        ...
        .flat()
        .map(m => m.trim());
    if (typeof metricNames === 'undefined' || metricNames === null || metricNames.length === 0 || typeof metricNames[0] !== 'string' || metricNames[0].trim().length === 0) {
    ...
    // Aggregation options
    let metricAggregates = (req.multiValueQueryStringParameters.agg || req.multiValueQueryStringParameters.aggregates || [])
        .map(agg => agg.split(','))
        .flat()
    ...
    try {
        ...
        */
        let metrics = metricNames.map((m, index) => {
            if (MULTI_DIM_METRICS[m]) {
                ...
                if (cacheMetricDimensions && cacheMetricDimensions.length) {
                    ...
                    return cacheMetricDimensions.map(t => ({
                        name: MULTI_DIM_METRICS[m].metric, dimensions: [{
...
async function getMetricData(cloudwatch, eventId, metrics, start, end, periodLength) {
    ...
    data.MetricDataResults.forEach(cwResult => {
        ...
        let values = aggregateValues(cwResult.Values.map((v, index) => ({
            value: v,
            date: cwResult.Timestamps[index]
        })), periodLength, needPostAggregation, agg);
...
async function getEventData(ddb, eventId) {
    ...
    return multiDimensions.reduce((acc, dim) => {
